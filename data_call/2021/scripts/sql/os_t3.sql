WITH nb_trips_by_harbour as (
		SELECT
			h.topiaid as harbour_id,
			h.locode::TEXT AS locode,
			count(DISTINCT t.topiaid)::integer as nb_trips
		FROM
			public.trip t
			JOIN public.harbour h on (t.landingharbour = h.topiaid)
		WHERE
			EXTRACT(YEAR FROM t.landingdate) IN (2018, 2019, 2020)
		GROUP BY
			harbour_id,
			locode
),
nb_trips_sampled_by_harbour as (
		SELECT
			h.topiaid as harbour_id,
			h.locode::TEXT AS locode,
			count(distinct t.topiaid)::integer as nb_trips
		FROM
			public.trip t
			JOIN harbour h on (t.landingharbour = h.topiaid)
			JOIN sample s on (s.trip = t.topiaid)
		WHERE
			EXTRACT(YEAR FROM t.landingdate) IN (2018, 2019, 2020)
		GROUP BY
			harbour_id,
